...
@ViewDeclaration( id = ChargingNonLiquidFurnaceDetailViewController.ID, fxmlFile = "/com/hydro/casting/gui/prod/view/detail/ChargingNonLiquidFurnaceDetailView.fxml", type = ViewType.DETAIL )
public class ChargingNonLiquidFurnaceDetailViewController extends ViewControllerBase
{
    public final static String ID = SecurityCasting.PROD.DETAILS.CHARGING_NON_LIQUID_FURNACE.VIEW;
    ...
    private Predicate<MaterialGroup> nonLiquidPredicate = ( materialGroup ) -> {
        if ( materialGroup.getName().equals( EMaterialGroup.SPEC_MATERIAL.getApk() ) || materialGroup.getName().equals( "Auflegierungen" ) || materialGroup.getName().equals( "Flüssigmetalle" ) )
        {
            return false;
        }
        return true;
    ...
    {
        materialBrowser.loadMaterials( nonLiquidPredicate );
    }
